...
SELECT * FROM PROJECT18.DOCTOR;

CREATE TABLE DOCTOR
AS 
SELECT * 
FROM PROJECT18.DOCTOR;
    
CREATE TABLE DOCTOR_DEGREES
AS 
    SELECT * 
    FROM PROJECT18.DOCTOR_DEGREES;


CREATE TABLE DOCTOR_CERT
AS 
    SELECT * 
    FROM PROJECT18.DOCTOR_CERT;


CREATE TABLE DOCTOR_SPECIALTIES
AS 
    SELECT * 
    FROM PROJECT18.DOCTOR_SPECIALTIES;
  
CREATE TABLE MENTOR_DOCTOR
AS 
    SELECT * 
    FROM PROJECT18.MENTOR_DOCTOR;

CREATE TABLE STAFF
AS 
    SELECT * 
    FROM PROJECT18.STAFF;
            
CREATE TABLE DOCTOR_ADMIN_ASSISTANT
AS 
    SELECT * 
    FROM PROJECT18.DOCTOR_ADMIN_ASSISTANT;
                
CREATE TABLE DEPARTMENT
AS 
    SELECT * 
    FROM PROJECT18.DEPARTMENT;

CREATE TABLE DOC_WORKS_AT
AS 
    SELECT * 
    FROM PROJECT18.DOC_WORKS_AT;

CREATE TABLE NURSE
AS 
    SELECT * 
    FROM PROJECT18.NURSE;

CREATE TABLE NURSE_SCHEDULE
AS 
    SELECT * 
    FROM PROJECT18.NURSE_SCHEDULE;

CREATE TABLE INSURANCE
AS 
    SELECT * 
    FROM PROJECT18.INSURANCE;

CREATE TABLE PATIENT
AS 
    SELECT * 
    FROM PROJECT18.PATIENT;

--select tablespace_name, table_name from SYS.ALL_TABLES;

CREATE TABLE PATIENT_ACCOUNT_SERVICES
AS
    SELECT *
    FROM PROJECT18.PATIENT_ACCOUNT_SERVICES;


CREATE TABLE PATIENT_ACCOUNT_PAYMENT
AS
    SELECT *
    FROM PROJECT18.PATIENT_ACCOUNT_PAYMENT;


CREATE TABLE PATIENT_MEDICINE_HISTORY
AS
    SELECT *
    FROM PROJECT18.PATIENT_MEDICINE_HISTORY;

CREATE TABLE PATIENT_HOSPITAL_HISTORY
AS
    SELECT *
    FROM PROJECT18.PATIENT_HOSPITAL_HISTORY;

CREATE TABLE PATIENT_ALLERGY_HISTORY
AS
    SELECT *
    FROM PROJECT18.PATIENT_ALLERGY_HISTORY;


CREATE TABLE PATIENT_INFECTION_HISTORY
AS
    SELECT *
    FROM PROJECT18.PATIENT_INFECTION_HISTORY;


CREATE TABLE WALK_IN_FOR_ADMISSION
AS
    SELECT *
    FROM PROJECT18.WALK_IN_FOR_ADMISSION;


CREATE TABLE PATIENT_ROOM
AS
    SELECT *
    FROM PROJECT18.PATIENT_ROOM;
...
AS
    SELECT *
    FROM PROJECT18.DOC_VISIT;
 
CREATE TABLE NURSE_VISIT
AS
    SELECT *
    FROM PROJECT18.NURSE_VISIT;
 
CREATE TABLE REQUEST_HOSPITALIZATION
AS
    SELECT *
    FROM PROJECT18.REQUEST_HOSPITALIZATION; 
 
CREATE TABLE PRESCRIPTION
AS
    SELECT *
    FROM PROJECT18.PRESCRIPTION; 
 
CREATE TABLE SUPPLIER
AS
    SELECT *
    FROM PROJECT18.SUPPLIER; 
 
CREATE TABLE MEDICAL_SUPPLIES
AS
    SELECT *
    FROM PROJECT18.MEDICAL_SUPPLIES; 

CREATE TABLE NON_MEDICAL_SUPPLIES
AS
    SELECT *
    FROM PROJECT18.NON_MEDICAL_SUPPLIES; 
   
CREATE TABLE REQUEST_MEDICAL_SUPPLIES
AS
    SELECT *
    FROM PROJECT18.REQUEST_MEDICAL_SUPPLIES; 
    
CREATE TABLE REQUEST_NON_MEDICAL_SUPPLIES
AS
    SELECT *
    FROM PROJECT18.REQUEST_NON_MEDICAL_SUPPLIES; 
    
CREATE TABLE OPERATING_ROOM
AS
    SELECT *
    FROM PROJECT18.OPERATING_ROOM;     
 
 
CREATE TABLE REQUEST_OPERATING_ROOM
AS
    SELECT *
    FROM PROJECT18.REQUEST_OPERATING_ROOM;  
...
-- chapter 3 - subprograms and packages
...
DECLARE
    L_VAR SYS_REFCURSOR;
    H_PATIENT PATIENT_PKG.HOSPITAL_RECORD;
BEGIN
    -- BASE CASE WHERE PROPER INPUT IS GIVEN
    DBMS_OUTPUT.put_line('BASE CASE WHERE PROPER INPUT IS GIVEN');
    PATIENT_PKG.TMP_OUTPUT_VAR:=PATIENT_PKG.PATIENT_INFO(1);
    DBMS_OUTPUT.put_line(PATIENT_PKG.TMP_OUTPUT_VAR.NAME||'   '||PATIENT_PKG.TMP_OUTPUT_VAR.SSN||'    '||PATIENT_PKG.TMP_OUTPUT_VAR.DR_LICENSE||'    '||
                            PATIENT_PKG.TMP_OUTPUT_VAR.ADDRESS||'    '||PATIENT_PKG.TMP_OUTPUT_VAR.PHONE||'    '||PATIENT_PKG.TMP_OUTPUT_VAR.LAST_SERVICE_DATE||'    '||
                            PATIENT_PKG.TMP_OUTPUT_VAR.SERVICE_TYPE||'    '||PATIENT_PKG.TMP_OUTPUT_VAR.BALANCE||' '||
                            PATIENT_PKG.TMP_OUTPUT_VAR.INSURANCE_NAME||' '||PATIENT_PKG.TMP_OUTPUT_VAR.INSURANCE_NUMBER);
    
    DBMS_OUTPUT.put_line('');
    
    -- WRONG INPUT BASE FUNCTION CALL
    DBMS_OUTPUT.put_line('WRONG INPUT BASE FUNCTION CALL');
    PATIENT_PKG.TMP_OUTPUT_VAR:=PATIENT_PKG.PATIENT_INFO(111);
    DBMS_OUTPUT.put_line(PATIENT_PKG.TMP_OUTPUT_VAR.NAME||'   '||PATIENT_PKG.TMP_OUTPUT_VAR.SSN||'    '||PATIENT_PKG.TMP_OUTPUT_VAR.DR_LICENSE||'    '||
                            PATIENT_PKG.TMP_OUTPUT_VAR.ADDRESS||'    '||PATIENT_PKG.TMP_OUTPUT_VAR.PHONE||'    '||PATIENT_PKG.TMP_OUTPUT_VAR.LAST_SERVICE_DATE||'    '||
                            PATIENT_PKG.TMP_OUTPUT_VAR.SERVICE_TYPE||'    '||PATIENT_PKG.TMP_OUTPUT_VAR.BALANCE);
                            
    -- OVERLOADED FUNCTION CALL USING SSN AS INPUT
    DBMS_OUTPUT.put_line('OVERLOADED FUNCTION CALL USING SSN AS INPUT');
    PATIENT_PKG.TMP_OUTPUT_VAR:=PATIENT_PKG.PATIENT_INFO('414433531');
    DBMS_OUTPUT.put_line(PATIENT_PKG.TMP_OUTPUT_VAR.NAME||'   '||PATIENT_PKG.TMP_OUTPUT_VAR.SSN||'    '||PATIENT_PKG.TMP_OUTPUT_VAR.DR_LICENSE||'    '||
                            PATIENT_PKG.TMP_OUTPUT_VAR.ADDRESS||'    '||PATIENT_PKG.TMP_OUTPUT_VAR.PHONE||'    '||PATIENT_PKG.TMP_OUTPUT_VAR.LAST_SERVICE_DATE||'    '||
                            PATIENT_PKG.TMP_OUTPUT_VAR.SERVICE_TYPE||'    '||PATIENT_PKG.TMP_OUTPUT_VAR.BALANCE||'  '||
                            PATIENT_PKG.TMP_OUTPUT_VAR.INSURANCE_NAME||' '||PATIENT_PKG.TMP_OUTPUT_VAR.INSURANCE_NUMBER);
    ...
    -- OVERLOADED FUNCTION CALL USING FIRST AND SECOND NAME AS INPUT
    DBMS_OUTPUT.put_line('OVERLOADED FUNCTION CALL USING FIRST AND SECOND NAME AS INPUT');
    PATIENT_PKG.TMP_OUTPUT_VAR:=PATIENT_PKG.PATIENT_INFO('Maryanna','Songust');
    DBMS_OUTPUT.put_line(PATIENT_PKG.TMP_OUTPUT_VAR.NAME||'   '||PATIENT_PKG.TMP_OUTPUT_VAR.SSN||'    '||PATIENT_PKG.TMP_OUTPUT_VAR.DR_LICENSE||'    '||
                            PATIENT_PKG.TMP_OUTPUT_VAR.ADDRESS||'    '||PATIENT_PKG.TMP_OUTPUT_VAR.PHONE||'    '||PATIENT_PKG.TMP_OUTPUT_VAR.LAST_SERVICE_DATE||'    '||
                            PATIENT_PKG.TMP_OUTPUT_VAR.SERVICE_TYPE||'    '||PATIENT_PKG.TMP_OUTPUT_VAR.BALANCE||'  '||
                            PATIENT_PKG.TMP_OUTPUT_VAR.INSURANCE_NAME||' '||PATIENT_PKG.TMP_OUTPUT_VAR.INSURANCE_NUMBER);
    ...
    DBMS_OUTPUT.put_line('');
    -- HOSPITALISED PATIENT - RECORD DOES NOT EXIST
    DBMS_OUTPUT.put_line('HOSPITALISED PATIENT - RECORD DOES NOT EXIST');
    L_VAR:=PATIENT_PKG.HOSPITALIZED_PATIENT(5);
    LOOP
        FETCH L_VAR INTO H_PATIENT;
        EXIT WHEN L_VAR%NOTFOUND;
        DBMS_OUTPUT.PUT_LINE(H_PATIENT.PATIENT_NAME||'    '|| H_PATIENT.OPERATION_DATE ||'   '||H_PATIENT.OPERATION_TYPE ||'   '||
        H_PATIENT.ROOM_NO ||'   '||H_PATIENT.DOCTOR_NAME ||'   '||H_PATIENT.DEPT  );
    END LOOP;    
    DBMS_OUTPUT.PUT_LINE('');
    ...
    DBMS_OUTPUT.put_line('');
    DBMS_OUTPUT.put_line('PATIENT_STATUS FOR PATIENT - Maryanna Songust');
    -- PATIENT STATUS
    DBMS_OUTPUT.put_line(PATIENT_PKG.PATIENT_STATUS('Maryanna','Songust'));
    
    --PATIENT_RELEASE - SSN AS INPUT
    DBMS_OUTPUT.put_line('');
    DBMS_OUTPUT.put_line('PATIENT RELEASE DATE FOR PATIENT (SSN INPUT) - Maryanna Songust');
    DBMS_OUTPUT.put_line(PATIENT_PKG.PATIENT_RELEASE('414433531'));
    
    --PATIENT_RELEASE - ID AS INPUT
    DBMS_OUTPUT.put_line('');
    DBMS_OUTPUT.put_line('PATIENT RELEASE DATE FOR PATIENT (ID INPUT) - Maryanna Songust');
    DBMS_OUTPUT.put_line(PATIENT_PKG.PATIENT_RELEASE(1));
    
    --PATIENT_RELEASE - NAME AS INPUT
    DBMS_OUTPUT.put_line('');
    DBMS_OUTPUT.put_line('PATIENT RELEASE DATE FOR PATIENT (NAME INPUT) - Maryanna Songust');
    DBMS_OUTPUT.put_line(PATIENT_PKG.PATIENT_RELEASE('Maryanna', 'Songust'));
    
    
    --PATIENT_MEDICAL_HISTORY - SSN AS INPUT
    DBMS_OUTPUT.put_line('');
    DBMS_OUTPUT.put_line('PATIENT MEDICAL HISTORY FOR  Maryanna Songust');
    DBMS_OUTPUT.put_line(PATIENT_PKG.PATIENT_MEDICAL_HISTORY(414433531));
    
    --ALLERGY_LIST - ALLERGY NAME AS INPUT
    DBMS_OUTPUT.put_line('');
    DBMS_OUTPUT.put_line('PATIENTS WITH THE GIVEN ALLERGY ');
    DBMS_OUTPUT.put_line(PATIENT_PKG.ALLERGY_LIST('POLLEN'));
...
SELECT Employee_pkg.OPERATION(5) FROM DUAL;

DECLARE
    L_VAR SYS_REFCURSOR;
    L_STATUS Employee_pkg.OR_RECORD;
    MEDIC_SUPPLY Employee_pkg.MEDICAL_SUPPLY_RECORD;
    NON_MEDIC_SUPPLY Employee_pkg.NON_MEDICAL_SUPPLY_RECORD;
    EMPLOYEE Employee_pkg.EMP_RECORD;
BEGIN
    DBMS_OUTPUT.PUT_LINE('OPEARTIONS ID = 5 - ');
    L_VAR:=Employee_pkg.OPERATION(5);
    LOOP
        FETCH L_VAR INTO L_STATUS;
        EXIT WHEN L_VAR%NOTFOUND;
        DBMS_OUTPUT.PUT_LINE(L_STATUS.DOCTOR_NAME||'    '|| L_STATUS.DEPT ||'   '||L_STATUS.OPERATION_DATE ||'   '||
        L_STATUS.OPERATION_DURATION ||'   '||L_STATUS.PATIENT_NAME ||'   '||L_STATUS.OPERATION_COST  );
    END LOOP;    
    DBMS_OUTPUT.PUT_LINE('');
    
    DBMS_OUTPUT.PUT_LINE('MEDICAL_SUPPLIES ID = 2 - ');
    L_VAR:=Employee_pkg.MEDICAL_SUPPLIES(2);
    LOOP
        FETCH L_VAR INTO MEDIC_SUPPLY;
        EXIT WHEN L_VAR%NOTFOUND;
        DBMS_OUTPUT.PUT_LINE(MEDIC_SUPPLY.STAFF_NAME||'    '|| MEDIC_SUPPLY.NAME ||'   '||MEDIC_SUPPLY.QUANTITY ||'   '||
        MEDIC_SUPPLY.BRAND_NAME ||'   '||MEDIC_SUPPLY.SUPPLIER_ID ||'   '||MEDIC_SUPPLY.SUPPLIER_NAME  );
    END LOOP;
    DBMS_OUTPUT.PUT_LINE('');
    
    DBMS_OUTPUT.PUT_LINE('NON_MEDICAL_SUPPLIES ID = 2 - ');
    L_VAR:=Employee_pkg.NON_MEDICAL_SUPPLIES(2);
    LOOP
        FETCH L_VAR INTO NON_MEDIC_SUPPLY;
        EXIT WHEN L_VAR%NOTFOUND;
        DBMS_OUTPUT.PUT_LINE(NON_MEDIC_SUPPLY.STAFF_NAME||'    '|| NON_MEDIC_SUPPLY.NAME ||'   '||NON_MEDIC_SUPPLY.QUANTITY ||'   '||
        NON_MEDIC_SUPPLY.BRAND_NAME );
    END LOOP;
    DBMS_OUTPUT.PUT_LINE('');
    
    DBMS_OUTPUT.PUT_LINE('MEDICAL_SUPPLIES_ORDER ID = 3 - ');
    IF Employee_pkg.MEDICAL_SUPPLIES_ORDER(3) THEN
    ...
    DBMS_OUTPUT.PUT_LINE('EMPLOYEE_LIST - ');
    L_VAR:=Employee_pkg.EMPLOYEE_LIST;
    LOOP
        FETCH L_VAR INTO EMPLOYEE;
        EXIT WHEN L_VAR%NOTFOUND;
        DBMS_OUTPUT.PUT_LINE(RPAD(EMPLOYEE.NAME,15,' ')||'    '|| EMPLOYEE.BIRTHDAY ||'   '||EMPLOYEE.EMPLOYEE_TYPE ||'   '||EMPLOYEE.SALARY );
    END LOOP;
    DBMS_OUTPUT.PUT_LINE('');
    
    DBMS_OUTPUT.PUT_LINE('EMPLOYEES WITH ID =1 ');
    L_VAR:=Employee_pkg.EMPLOYEE_LIST(1);
    LOOP
        FETCH L_VAR INTO EMPLOYEE;
        EXIT WHEN L_VAR%NOTFOUND;
        DBMS_OUTPUT.PUT_LINE(RPAD(EMPLOYEE.NAME,15,' ')||'    '|| EMPLOYEE.BIRTHDAY ||'   '||EMPLOYEE.EMPLOYEE_TYPE ||'   '||EMPLOYEE.SALARY );
...
BEGIN
-- SUCCESSFUL DATA INSERT WITH THE LEAST MINIMUM AMOUNT OF DATA FOR THE TABLE
INSERT_pkg.PATIENT_INSERT('JANE','DOE',123456689,0,'','','',12532,'06-FEB-2008',4500896504,'','','','','','','','','NM','M','','N',
                            'N',15,101,150089,'MD');

-- SUCCESSFUL DATA INSERT
INSERT_pkg.ACCOUNT_CHARGES_PATIENT_INSERT(10,'06-FEB-2018','GENERAL_CHECKUP','OUTPATIENT',200);

-- NO INPUT, ERROR IS NOT PRODUCED SINCE THERE ARE EXCEPTION HANDLERS
INSERT_pkg.PATIENT_HISTORY_INSERT('','','','','','','','');
END;
/
...
-- INSERT THIS PROCEDURE IN EXCEPTION HANDLERS
...
select * from doctor;
select * from DOCTOR_DEGREES;
select * from doctor_cert;
select * from DOCTOR_SPECIALTIES;
select * from MENTOR_DOCTOR;
select * from staff;
select * from DOCTOR_ADMIN_ASSISTANT;
select * from DEPARTMENT;
select * from DOC_WORKS_AT;
select * from nurse;
...
select * from patient;
select * from PATIENT_ACCOUNT_SERVICES;
select * from PATIENT_ACCOUNT_PAYMENT;
select * from PATIENT_MEDICINE_HISTORY;
select * from PATIENT_HOSPITAL_HISTORY;
select * from PATIENT_ALLERGY_HISTORY;
select * from PATIENT_INFECTION_HISTORY;
select * from WALK_IN_FOR_ADMISSION;
select * from PATIENT_ROOM;
select * from bed;
select * from NURSE_VISIT;
select * from DOC_VISIT;
select * from REQUEST_HOSPITALIZATION;
select * from PRESCRIPTION;
select * from SUPPLIER;
select * from MEDICAL_SUPPLIES;
select * from NON_MEDICAL_SUPPLIES;
select * from REQUEST_MEDICAL_SUPPLIES;
select * from REQUEST_NON_MEDICAL_SUPPLIES;
select * from OPERATING_ROOM;
select * from REQUEST_OPERATING_ROOM;
